...
#### Проверка через бутстреп (Медианы):

Используем бутстреп, чтобы создать выборку из бустрепных медиан. Всего таких медиан будет 100.000, то есть получили выборку медиан. 

Распределение бустрепных медиан (апрель-июнь):

![Тут](./IMAGES/bs_med_hist_apr_jun.png)

Распределение бустрепных медиан (июль-март):

![Тут](./IMAGES/bs_med_hist_jul_mar.png)

Далее проведем t-test для бустрепных медиан.

Получили, что p-value стремиться к 0. Значит между медианами есть статистичкая значимость, это не случайность
